...
export default function Starfield() {
  ...
  useEffect(() => {
    ...
    function spawnEnemy() {
      ...
      const vx = (toCX / len) * ENEMY_SPEED;
      const vy = (toCY / len) * ENEMY_SPEED;
    ...
    function enemyFire(e) {
      ...
      bullets.push({
        ...
        life: BULLET_LIFE,
        color: e.fireEvery < 550 ? "red" : "blue"
      });
    }

    // player
    function playerFire(colorOverride) {
      myBullets.push({
        x: cursorX, y: cursorY,
        vx: 0, vy: -MY_BULLET_SPEED,
        life: MY_BULLET_LIFE,
    ...
    function frame(tNow) {
      ...
      for (const s of stars) {
        ...
        s.z -= SPEED * s.z * s.z;
        project(s);
        if (s.z <= Z_MIN || s.sx < -120 || s.sx > W + 120 || s.sy < -120 || s.sy > H + 120) {
      ...
      if (spawnTimer >= nextSpawnIn) {
        spawnTimer = 0;
        nextSpawnIn = randBetween(ENEMY_MIN_SPAWN_MS, ENEMY_MAX_SPAWN_MS);
        spawnEnemy();
      }
      ...
      for (const pb of myBullets) {
        ...
        else ctx.strokeStyle = "rgba(120, 255, 140, 0.95)"; // green default
        ctx.lineWidth = 3;
        ctx.beginPath();
